<?php 
	...
	function StoreData($exploration_date, $guild_name, $guild_invite, $expiration_date, $guild_members_count, $guild_members_online, $guild_id, $guild_icon, $guild_banner, $guild_description, $verification_level, $invite_channel_id, $invite_channel_name, $inviter_id, $inviter_name, $inviter_avatar){
		
		GLOBAL $con;
 		$sql = "SELECT guild_id FROM server_list WHERE guild_id=?";
     	$st=$con->prepare($sql);
		$st->execute(array($guild_id));
		$all=$st->fetchAll();
		if (count($all) == 1){
			...
			GLOBAL $con;
			$sql = "INSERT INTO server_list (exploration_date, guild_name, guild_invite, expiration_date, guild_members_count, guild_members_online, guild_id, guild_icon, guild_banner, guild_description, verification_level, invite_channel_id, invite_channel_name, inviter_id, inviter_name, inviter_avatar) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
			$st=$con->prepare($sql);
			$st->execute([$exploration_date, $guild_name, $guild_invite, $expiration_date, $guild_members_count, $guild_members_online, $guild_id, $guild_icon, $guild_banner, $guild_description, $verification_level, $invite_channel_id, $invite_channel_name, $inviter_id, $inviter_name, $inviter_avatar]);
			http_response_code(200);
